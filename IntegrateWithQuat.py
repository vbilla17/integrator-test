...
def multiplyQuat(q1 : Quaternion, q2 : Quaternion):
    q = Quaternion()
...
def quaternionRotation(q, v):
    # q is a quaternion, v is a vector
    # v' = qvq*
    ...
    v_prime = multiplyQuat(multiplyQuat(q, v_prime), q_conj)

    return [v_prime.x, v_prime.y, v_prime.z]
...
# a is the 3d acceleration vector
# q is the 4d quaternion
def integrateAccelQuat(start_alt, a, q, accel_timestamps, quat_timestamps):
    last_accel = 0
    last_timestamp = accel_timestamps[0]
    current_vel = 0
    current_alt = start_alt
    ...
    ratioQuatAccel = len(quat_timestamps) / len(accel_timestamps)
    for i in range(1, len(accel_timestamps)):

        current_accel = quaternionRotation(q[int(ratioQuatAccel * i)], a[i])[2]
        print("AQ:",current_accel)
        print("QUAT",q[int(ratioQuatAccel * i)],i, int(ratioQuatAccel * i))
        # TODO: figure out why this is the case for BNO data
        if(i <= 1599):
            current_accel = -current_accel
        accelsZ.append(current_accel)
        current_timestamp = accel_timestamps[i]
        delta_t = (current_timestamp - last_timestamp)
        current_vel += delta_t * ((last_accel + current_accel) - (2 * gravVal(current_alt))) / 2
        last_accel = current_accel
    ...
    return velocities, altitudes, accelsZ
...
def correctAccelAll(aAll, flight, sensor):
    cv= 0
    if flight == "Jawbone":
        if sensor == "ADIS":
            cv = -0.03217405
...
# input all ax, ay, az data
def inputAccelAll(flight, sensor):
    if flight == "Jawbone":
        if sensor == "ADIS":
            accel_input_path = './input/Jawbone/ADIS-trimmed.csv'
        elif sensor == "ACCEL":
    ...
    with open(accel_input_path, 'r') as accel_file:
        ...
        for row in accel_reader:
            if row[checksum_idx] == 'OK':
                accel_timestamps.append(float(row[power_ctr_idx]))
                a.append([float(row[ax_idx]), float(row[ay_idx]), float(row[az_idx])])
                accel_packets.append(float(row[packet_idx]))

        if not (len(accel_timestamps) == len(a)):
            print("Acceleration and timestamp lists are different lengths!!")
            return 1
        else:
            print("Accel data read!")

        return a, accel_packets, accel_timestamps
...
def inputQuat(flight):
    if flight == "Jawbone":
        bno_input_path = 'input/Jawbone/BNO-trimmed.csv'

    print("Reading baro data from ", bno_input_path)
    ...
    with open(bno_input_path, 'r') as bno_file:
        ...
        checksum_idx = headers.index('Checksum Status')
        qX_idx = headers.index('quatX')
        qY_idx = headers.index('quatY')
        qZ_idx = headers.index('quatZ')
        qW_idx = headers.index('quatW')
        power_ctr_idx = headers.index('pwr_ctr')

        for row in bno_reader:
            if row[checksum_idx] == 'OK':
                bno_timestamps.append(float(row[power_ctr_idx]))
                qOut = Quaternion()
                qOut.x = float(row[qX_idx])
                qOut.y = float(row[qY_idx])
                qOut.z = float(row[qZ_idx])
                qOut.w = float(row[qW_idx])
                q.append(qOut)

        if not (len(bno_timestamps) == len(q)):
            print("BNO and timestamp lists are different lengths!!")
            return 1
        else:
            print("BNO data read!")

        return q, bno_timestamps
...
def makePlots(az, velocities, altitudes, accel_timestamps, gps_alt, gps_timestamps, baro_alt, baro_timestamps, vQ, aQ):
    ...
    plt.plot(accel_timestamps, vQ)
    plt.title("Velocity")

    plt.figure(figsize=(10, 8))
    plt.plot(accel_timestamps, altitudes, label = "integrated")
    plt.plot(accel_timestamps, aQ, label = "integratedQ")
...
def main():
    ...
    flight = "Jawbone" # input("Enter the flight name to plot: ")
    sensor = "ACCEL" # input("Enter the sensor type to use (ADIS/ACCEL): ")

    if isValidInput(flight, sensor):

        az, accel_packets, accel_timestamps = inputAccel(flight, sensor)
        aAll, aAll_packets, aAll_timestamps = inputAccelAll(flight,sensor)
        gps_alt, gps_timestamps = inputGps(flight)
        baro_alt, baro_timestamps = inputBaro(flight)
        quat, bno_timestamps = inputQuat(flight)

        timescale = setTimescale(flight)

        az, accel_timestamps = trimData(az, accel_timestamps, timescale[0], timescale[1], timescale[2])
        aAll, aAll_timestamps = trimData(aAll, aAll_timestamps, timescale[0], timescale[1], timescale[2])
        ...
        quat, bno_timestamps = trimData(quat, bno_timestamps, timescale[0], timescale[1], timescale[2])

        correctAccel(az, flight, sensor)
        correctAccelAll(aAll, flight, sensor)
        correctGps(gps_alt, flight)
        correctBaro(baro_alt)

        velocities0, altitudes0 = integrateAccel(gps_alt[0], az, accel_timestamps)
        velocities, altitudes, accelZ = integrateAccelQuat(gps_alt[0], aAll, quat, aAll_timestamps, bno_timestamps)

        plt.plot(az, label="Original Acceleration Z")
        plt.plot(accelZ, label="Transformed Acceleration Z")
        plt.xlabel("s")
        plt.ylabel("ft/s^2")
        plt.legend()
        plt.show()
      ...
      #  printMaxes(az, velocities, altitudes, accel_timestamps, gps_alt, gps_timestamps, baro_alt, baro_timestamps)
        ...
        makePlots(az, velocities0, altitudes0, accel_timestamps, gps_alt, gps_timestamps, baro_alt, baro_timestamps, velocities, altitudes)
    else:
